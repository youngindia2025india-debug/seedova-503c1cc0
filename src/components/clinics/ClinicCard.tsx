import { BadgeCheck, GitCompare, Heart, MapPin, Star, TrendingUp } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { cn } from "@/lib/utils";
import { formatCost, type Clinic } from "./data";
...
export function ClinicCard({ clinic, view, saved, compared, onToggleSave, onToggleCompare }: Props) {
    ...
    >
      <div className={cn("relative overflow-hidden", isList ? "sm:w-72 sm:shrink-0" : "")}>
        ...
        <button
          ...
          className="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full bg-background/85 text-foreground shadow-sm backdrop-blur transition-transform hover:scale-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-90"
        >
          <Heart
            className={cn(
      ...
      <div className="flex flex-1 flex-col gap-4 p-5 sm:p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-2xl bg-secondary/70 px-3 py-2.5">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Est. IVF cost</p>
            <p className="mt-0.5 text-sm font-semibold text-foreground">
              {formatCost(clinic.costMin)} – {formatCost(clinic.costMax)}
            </p>
          </div>
          <div className="rounded-2xl bg-secondary/70 px-3 py-2.5">
